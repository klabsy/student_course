    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="get_studentinfo(scope.$index, scope.row)">查看</el-button>
          <el-button
            size="mini"
...
<!--    添加-->
    <el-dialog title="添加科目" :visible.sync="dialogaddFormVisible" width="30%">
      <el-form :model="a_info">
        <el-form-item label="科目名" :label-width="formLabelWidth">
          <el-input v-model="a_info.cname" ></el-input>
        </el-form-item>
        <el-form-item label="Cpno" :label-width="formLabelWidth">
          <el-input v-model="a_info.cpno" ></el-input>
        </el-form-item>
        <el-form-item label="Ccredit" :label-width="formLabelWidth">
          <el-input v-model="a_info.ccredit" ></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods:{
    ...
    addinfo() {
      ...
      this.$http.post('/api/student_course_springMVC_war_exploded/course/add_Course',this.a_info)
        .then(res => {
          // this.info = res.data
          // console.log(this.info)
          console.log("test")
    ...
    delinfo(index, row) {
      ...
      console.log(row.cno);
      this.$http.delete('/api/student_course_springMVC_war_exploded/course/del_Course/'+row.cno)
        .then(res => {
          this.get_info()
          // this.info = res.data
          // console.log(this.info)
