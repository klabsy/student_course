...
<!--    添加-->
    <el-dialog title="添加课程" :visible.sync="dialogaddFormVisible" width="30%">
      <el-form :model="a_info">
        <el-select v-model="sno" placeholder="学生">
          <el-option
...
export default {
  ...
  beforeCreate() {
    ...
    this.$http.post('/api/student_course_springMVC_war_exploded/sc/Cno_UpdateSc/'+this.$route.query.Cno)
      .then(res => {
        this.info = res.data
  ...
  methods:{
    ...
    addinfo() {
      ...
      this.a_info.cno = this.$route.query.Cno
      this.a_info.sno = this.sno
    ...
    delinfo(index, row) {
      ...
      console.log(row.cno);
      this.$http.delete('/api/student_course_springMVC_war_exploded/sc/del_Sc/'+ row.sno +"/"+ this.$route.query.Cno)
        .then(res => {
          this.get_info()
          // this.info = res.data
          // console.log(this.info)
